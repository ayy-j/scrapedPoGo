...
- **Entry points** live in `src/scrapers/`: `scrape.js` (basic data), `detailedscrape.js` (event details), `combinedetails.js` (merge temp into events), `scrapeShinies.js` (shiny list), `combineContextual.js` (cross-source aggregation).
...
- **Contextual aggregation** in `scrapers/combineContextual.js` merges `events/raids/eggs/research/rocketLineups/shinies` into `data/contextual.json` + `.min.json`.
...
- `npm run combinecontextual` → build `data/contextual.json`.
