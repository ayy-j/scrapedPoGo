...
- **Runtime**: Node.js 20 (CommonJS `require()` — no ESM)
- **DOM parsing**: jsdom 27 — all HTML extraction via JSDOM
...
- **No test framework** — validation via `npm run validate` against schemas
...
- `src/utils/logger.js` — CLI logger with `info`, `success`, `warn`, `error`, `start` methods
- `src/utils/blobUrls.js` — URL mapping from external CDNs to Vercel Blob storage
- `src/utils/blobNaming.js` — Blob pathname scheme logic (semantic folder structure)
- `src/utils/imageDimensions.js` — Batch image dimension fetching with caching
- `src/scripts/` — Utility scripts: `validate-schemas.js`, `upload-images-to-blob.js`, `download-images-locally.js`
- `schemas/` — JSON Schema definitions for all data types (eggs, events, raids, research, rocketLineups, shinies)
- `data/` — Output JSON files (`.min.json` for production, `.json` for readability)
- `data/eventTypes/` — Per-eventType files auto-generated by `combinedetails.js`
- `dataDocumentation/` — Endpoint documentation (one `.md` per data type + eventType-specific docs)
...
Stage 3: npm run combinedetails
  src/scrapers/combinedetails.js → merges temp files into events, flattens structure, generates per-type files
...
| `extractPokemonList(container, options?)` | Parse `.pkmn-list-flex` containers → `Pokemon[]` |
| `extractBonuses(doc)` | Parse bonus sections → `{ bonuses, disclaimers }` |
...
| `extractSection(doc, sectionId)` | Get content between section headers → `{ paragraphs, lists, pokemon, tables }` |
| `getSectionHeaders(doc)` | List all section IDs on a page |
| `extractPrice(text)` | Parse price from text → `{ price, amount, currency }` |
...
| `parseBonusMultiplier(text)` | Parse "2×" style multipliers |
| `parseGBLCups(...)` | Parse GO Battle League cup data |

### Adding a new eventType scraper

1. Create `src/pages/detailed/{eventtype}.js` — follow `communityday.js` pattern
2. Register in `src/scrapers/detailedscrape.js` if/else chain (add `else if` block)
3. Add event type slug to `schemas/events.schema.json` → `eventType.enum` array
4. Add merge case in `src/scrapers/combinedetails.js` → the type string matching block
5. Document in `dataDocumentation/eventTypes/{EventType}.md`

## Data Conventions

### Event structure (flat — no nested `details` wrapper)
...
- ✅ **Always use `textContent` (not `innerHTML`)** when extracting text for data output — prevents Stored XSS from scraped HTML
- ✅ Use `sanitizeFilename()` for any user/scraped input used in file paths
- ✅ `getJSDOM(url)` / `fetchUrl(url)` include timeout and custom User-Agent
...
This makes the pipeline resilient — individual event scrape failures don't break the whole dataset.
...
- ✅ **Do**: Use existing `scraperUtils.js` functions — don't reinvent extraction logic
- ✅ **Do**: Follow the `get(url, id, bkp)` signature for all detailed scrapers
- ✅ **Do**: Use `writeTempFile()` in Stage 2 scrapers and let Stage 3 handle merging
- ❌ **Don't**: Nest data under a `details` wrapper — `combinedetails.js` flattens everything to top level
- ❌ **Don't**: Use `innerHTML` for text extraction (XSS risk) — use `textContent` instead
- ❌ **Don't**: Skip registering new event types in both `detailedscrape.js` AND `combinedetails.js`
