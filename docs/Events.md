...
## Per-EventType Files (Backwards Compatibility)

Each `eventType` also has its own file containing only events of that type. These files mirror the event objects in `events.min.json`, but are filtered to a single `eventType`. They are automatically generated by `combinedetails.js`.
