...
Images referenced in the scraped data can optionally be stored in [Vercel Blob Storage](https://vercel.com/docs/vercel-blob) for self-hosted image delivery.

### Setup

1. Create a Blob store in your Vercel project (Storage → Blob)
...
Set the `USE_BLOB_URLS=true` environment variable to transform image URLs in JSON output to use Blob storage URLs instead of external CDNs.

The URL mapping is stored in `data/blob-url-map.json` and automatically updated during uploads.

See [projectPlans/vercel/blob-storage-migration.md](projectPlans/vercel/blob-storage-migration.md) for detailed setup instructions.
